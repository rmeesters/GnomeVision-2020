...
VisionResultsPackage calculate(const cv::Mat &bgr, cv::Mat &processedImage){
	...
	Point2d cam_center = cv::Point2d(bgr.cols/2, bgr.rows/2);
	...
	cv::solvePNP(model_points, all_points, camera_matrix, dist_coeffs, rotation_vector, translation_vector);
    ...
    res.valid = true;
    res.area = largestarea;
    res.imgangle = returnangle;
...
VisionResultsPackage processingFailurePackage(ui64 time) {
    ...
    failureResult.area = -1;
    failureResult.imgangle = -1;
